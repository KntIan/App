		...
		<div class="group" :class="[!isPC ? 'group-h5' : '']">
			<div class="group-box">
				...
				<ul v-if="!groupInfo.isEdit" class="group-list">
					...
					<ul>
						<li v-for="(item, index) in createInfo" :key="index" class="group-list-item">
							...
							<input v-if="isPC" v-model="groupInfo.profile[item.key]" type="text"
				...
				<div v-else class="group-list group-list-edit">
					<input v-if="groupInfo.groupConfig.type === 'input'" v-model="groupInfo.groupConfig.value"
						class="group-name-input" type="text"
