<template>
  ...
  <timeline>
    <timelineItem v-if="matchDetail.match_date.data[0]" :leftTime='matchDetail.match_date.data[0].title' color=" #84F257">
      <view class="tips">{{matchDetail.match_date.data[0].s_date}}——{{matchDetail.match_date.data[0].e_date}}</view>
    </timelineItem>
    <timelineItem v-if="matchDetail.match_date.data[1]" :leftTime='matchDetail.match_date.data[1].title' color="#84F257">
      <view class="tips">{{matchDetail.match_date.data[1].s_date}}——{{matchDetail.match_date.data[1].e_date}}</view>
    </timelineItem>
    <timelineItem v-if="matchDetail.match_date.data[2]" :leftTime='matchDetail.match_date.data[2].title' :leftlist1="matchDetail.match_date.data[2].title" :leftlist2="matchDetail.match_date.data[2].title" color="#F25757">
      <view class="tips">{{matchDetail.match_date.data[2].s_date}}——{{matchDetail.match_date.data[2].e_date}}</view>
    </timelineItem>
    <timelineItem v-if="matchDetail.match_date.data[3]" :leftTime='matchDetail.match_date.data[3].title' :leftlist1="matchDetail.match_date.data[3].title" :leftlist2="matchDetail.match_date.data[3].title" color="#F25757">
  ...
  <view class="event_site">
    ...
    <view>
      <scroll-view class="scroll-view_H" scroll-x="true" @scroll="scroll" scroll-left="120" show-scrollbar="true">
        <view id="demo1" class="scroll-view-item_H uni-bg-red" v-for="(item, index) in matchDetail.match_scene" :key="index"><img :src="item.thumb" alt="" />
          <view class="img_text">
            {{item.title}}
          </view>
        </view>
      ...
      try {
        ...
        if (this.matchDetail.match_news && Array.isArray(this.matchDetail.match_news)) {
          this.matchDetail.match_news.forEach((item) => {
            item.titleimg = 'https://lanhu.oss-cn-beijing.aliyuncs.com/SketchPng73002970eb6951969634d1ac818f9fbd983ad92340d21cf601ab0d495647806c'; // 更改为实际的动态链接
            item.timeImg = 'https://lanhu.oss-cn-beijing.aliyuncs.com/SketchPng9124bf95026b34308d6bd993d7c3774578aa0f9fdf293cc08d4cb043bc1ba5a8'; // 更改为实际的动态链接
          });
        }
