...
<<<<<<< HEAD
	<view>
		...
		<uv-datetime-picker ref="datetimePicker" v-model="value" mode="date" :minDate="946695809000"
			@confirm="confirm" />
			...
			<view class="form">
				...
				<view style="margin-top: 24rpx" class="textlogin">
					<view class="img1" @click="selectGender('男')">
						<image v-if="isMale" src="../../static/img/boy1.png" alt="" />
						<image v-else src="../../static/img/boy.png" alt="" />
						<view class="imgtext1"> 男 </view>
					</view>
					<view class="img2" @click="selectGender('女')">
						<image v-if="isFemale" src="../../static/img/girl1.png" alt="" />
						<image v-else src="../../static/img/girl.png" alt="" />
				...
				<view class="input">
					<picker @change="giftPickerChange" :value="giftIndex" :range="giftArray.map((item) => item.title)"
						:range-key="'gift'">
				...
				<view class="input">
					<picker mode="selector" @change="onClassChange" :value="multiIndex"
						:range="multiArray.map((item) => item.title)" :disabled="isClassPickerDisabled">
				...
				<input type="text" placeholder="输入手机号" class="input" autocomplete="off"
					@input="updateField('iphone', $event)" />
				<view style="position: relative">
					<view class="title">验证码</view>
					<!-- autocomplete="off" off/nope 防止自动填充 -->
					<input type="text" placeholder="验证码" class="input" autocomplete="off"
						@input="updateField('captcha', $event)" />
					<view style="position: absolute;right: 0;
    bottom: 4px;
}" @click.stop="requestVerificationCode" :disabled="isCodeSent"
				...
				<input type="password" placeholder="输入密码" class="input" autocomplete="off"
...
<script setup>
	...
	const getlogin = async () => {
		...
		// 计算实际年龄
		const birthDate = new Date(dateYear.value);
		...
		// 比较计算出的年龄与用户输入的年龄
		if (calculatedAge !== parseInt(old.value, 10)) {
			uni.showToast({
				title: '输入的年龄与出生日期不匹配',
				icon: 'none',
				duration: 2000,
