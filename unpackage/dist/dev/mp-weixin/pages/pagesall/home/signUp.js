...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return {
    ...
    b: common_vendor.f($data.listdata, (item, index, i0) => {
      return {
        a: item.techerimg,
        b: common_vendor.t(item.biaoti),
        c: common_vendor.t(item.planum),
        d: common_vendor.t(item.quenum),
        e: item.hearimg,
        f: common_vendor.t(item.teachername),
        g: common_vendor.t(item.hours),
        h: common_vendor.t(item.minutes),
        i: common_vendor.t(item.seconds),
        j: common_vendor.t(item.pers),
        k: common_vendor.t(item.money == "免费" ? "免费" : "￥" + item.money),
        l: common_vendor.o((...args) => $options.btnsignup && $options.btnsignup(...args), index),
        m: index
      };
    })
  };
