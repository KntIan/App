...
// 获取班级数据方法
export const fetchClassData = (campusId) => {
  // 创建查询参数，通常需要将 campusId 放入查询字符串中
  const params = new URLSearchParams({ campusId }).toString() // 创建查询字符串
  return fetchData(`/user/classly?${params}`, 'GET') // 将 campusId 加入到 API 请求中
}

// 获取校区数据方法
export const fetchCampusData = () => fetchData('/user/school', 'GET', null)
