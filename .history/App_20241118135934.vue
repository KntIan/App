...
export default {
  ...
  methods: {
    getAreaSize() {
      ...
      const appElement = query.select("#app");
      if (appElement) {
        appElement.style.paddingTop = `${res.safeArea.top}px`; // 应用顶部安全距离
        appElement.style.paddingBottom = `${res.safeArea.bottom}px`; // 应用底部安全距离
      }
...
#app {
  padding-top: var(--safe-area-top); /* 使用自定义变量 */
}
</style>