...
import message from '../TUIKit/locales/en/message'
...
export const useStore = defineStore('main', {
  ...
  actions: {
    ...
    setListdataIndex(payload) {
  console.log('payload:', typeof payload); // 打印原始 payload 的类型

  // 检查 payload 是否为数组，如果不是，则尝试进行转换
  if (Array.isArray(payload)) {
    this.listdataIndex = payload; // 如果已经是数组，直接赋值
  } else if (payload instanceof Object) {
    // 如果 payload 是对象，则将其转换为数组
    this.listdataIndex = Object.values(payload); // 或者可以用其他方法转换为数组
  } else {
    // 如果是其他类型（如 null、undefined），可以设置为一个空数组
    this.listdataIndex = [];
    console.error('Invalid payload for listdataIndex, resetting to empty array:', payload);
  }
}
    ...
    setResultDiv(div) {
      ...
      console.log('resultDivData:', this.resultDivData)
    },
