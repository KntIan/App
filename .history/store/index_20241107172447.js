  ...
  actions: {
    ...
    async login(adminlist) {
      try {
        ...
        if (response.code === 1) {
          const { token, userinfo } = response.data.userinfo // 解构获取 token 和 userinfo
          console.log(userinfo)
          this.token = token // 存储登录成功后的 token
          uni.setStorageSync('token', token) // 将 token 存储在 localStorage
          // 登录成功后请求腾讯IM获取clientID
          // const clientIDResponse = await api.getClientID(user_id)
          // console.log('获取clientID成功:', clientIDResponse)
