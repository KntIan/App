...
const loadClasses = async () => {
  try {
    const response = await fetchClassData({ school_id: campusId.value });
...
const requestVerificationCode = async () => {
  ...
  try {
    ...
    console.log(response);
    // 假设接口返回的格式为 { success: true, message: '验证码发送成功' }
    if (response.code === 1) {
      isCodeSent.value = true; // 更新验证码发送状态
      startCountdown(); // 启动倒计时
      uni.showToast(response.msg || '验证码发送成功', 'success'); // 使用接口返回的消息
    } else {
      // 如果接口返回的 success 为 false，显示错误信息
      uni.showToast(response.msg || '发送验证码失败，请重试', 'error');
    }
  } catch (error) {
    // 捕获异常，显示通用错误信息
    uni.showToast('发送验证码失败，请重试', 'error');
    console.error('发送验证码错误:', error);
  } finally {
    uni.hideLoading(); // 无论成功与否都隐藏加载提示
  }
