  ...
  <view class="container">
    <view class="form">
      ...
      <view style="position: relative;">
        ...
        <!-- autocomplete="off" off/nope 防止自动填充 -->
        <input type="text" placeholder="输入手机号" class="input" autocomplete="off" @input="updateField('captcha', $event)" />
        <view style="position: absolute;" @click.stop="requestVerificationCode" :disabled="isCodeSent" :class="{ 'code-button': true, 'disabled-button': isCodeSent }">
...
import { fetchCampusData, fetchVerificationCode } from "@/utils/api";
...
const loadClasses = async () => {
  try {
    const response = await getRequest(`/user/classly?school_id=${campusId.value}`);
