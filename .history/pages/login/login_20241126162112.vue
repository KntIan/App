<template>
  <view @scroll.passive="handleScroll">
...
const handleLoginInfo = async () => {
  ...
  if (response.code === 1) {
    ...
    const { token } = response.data.registrationInformation

    store.setUserinfo(response.data.registrationInformation)
  ...
  const { user_id } = response.data.registrationInformation
  console.log(user_id)
