<template>
  ...
  <view class="container">
    <view class="form">
      ...
      <view class="input">
        <picker @change="giftPickerChange" :value="giftIndex" :range="giftArray" :range-key="'gift'">
          <view class="uni-input">{{ giftmoren }}</view>
        </picker>
      </view>
      <view class="title">班级</view>
      <view class="input">
        <picker mode="selector" @change="onClassChange" :value="multiIndex" :range="multiArray">
...
const loadClasses = async () => {
  try {
    const response = await fetchClassData(school_id.value); // 将 campusId 传入
...
const loadCampus = async () => {
  try {
    ...
    if (response && response.items) {
      giftArray.value = response.items.map((item) => ({
        gift: item.title,
      }));
...
const giftPickerChange = (event) => {
  ...
  giftmoren.value = giftArray.value[giftIndex.value].gift;

  // 更新 campusId 为选择的校区 ID
  school_id.value = giftArray.value[giftIndex.value].id; // 假设 id 是校区的唯一标识符
};
