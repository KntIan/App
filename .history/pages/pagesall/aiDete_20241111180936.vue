...
async function handleChooseFile(file) {
  try {
    console.log(file); // 打印传入的文件
    let blob = await compressImageFile(file); // 压缩文件
    console.log('127', blob); // 打印 blob
    let imageData = await file2ImageData(blob); // 获取图像数据
    console.log('129', imageData); // 如果这一行没有打印，说明 file2ImageData 出错
    let data = await evaluationImageData(imageData); // 确保获取有效数据
    data.filename = file.name; // 设置数据文件名
    await displayResults(data); // 展示结果
    console.log("测评结束", data); // 打印测评结束的信息
  } catch (error) {
    console.error('处理文件时发生错误:', error); // 捕捉并打印错误信息
  }
}
