  ...
  <view class="homePage">
    ...
    <view class="actiClass">
      ...
      <view class="actitme" v-for="(item, index) in listdata" :key="index">
        ...
        <view class="linetext1">{{ item.online }}</view>
...
const fetchData = () => {
  getRequest(`/activity/list?limit=7&page=1&token=${token}`)
    .then((res) => {
      console.log(res);
      // images.value = res.data.list.map((item) => item.image.join(","));
      // excellentWorks.value = res.data.list.map((item) => item.task_file);
      // studentShowcase.value = res.data.list.map((item) => item.avatar);
