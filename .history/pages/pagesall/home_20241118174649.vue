...
const hours = 0; // 示例：当前页面
const minutes = 0; // 示例：每页数据条数
const seconds = 0; // 示例：是否是首页数据
...
onMounted(() => {
  // const ces = getApp() // 获取全局应用实例
  // console.log('全局数据:', ces.globalData) // 打印全局数据
  fetchData()
  // startCountdown()
  updateCountdown()
})
  ...
  listdata.value.forEach((item, index) => {
    if (!isNaN(item.end_time)) {
      ...
      if (timeDiff > 0) {
        ...
        hours.value = Math.floor(totalSeconds / 3600); // 保持为数字
        minutes.value = Math.floor((totalSeconds % 3600) / 60); // 保持为数字
        seconds.value = totalSeconds % 60; // 保持为数字
        console.log('item.hours', hours.value);
        console.log('item.minutes', minutes.value);
        console.log('item.seconds', seconds.value);
      } else {
    ...
    // 更新 Store 的 listdata
    store.listdata[index] = item; // 使用索引更新对应的 item
    console.log(item, store.listdata[index])
  });
}
...
watch(listdata, (newListdata) => {
  console.log('Listdata updated:', newListdata);
});
...
watch(infoList, (newInfoList) => {
  console.log('InfoList updated:', newInfoList);
});
</script>
