...
const computedListData = computed(() => {
  return listdata.value.map(item => {
    const timeDiff = item.end_time - now; // 计算当前时间
    return {
      ...item,
      hours: Math.max(0, Math.floor(timeDiff / 3600)),
      minutes: Math.max(0, Math.floor((timeDiff % 3600) / 60)),
      seconds: Math.max(0, timeDiff % 60)
    };
  });
});
const btnscan = () => {
