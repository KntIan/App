...
onMounted(() => {
  console.log('resultsContainerRef:', resultsContainerRef.value); // 这里应该能获取到DOM元素
});
onLoad(() => {
  handleLogin();
});
...
async function evaluationImageData(imgData) {
  return new Promise(async (r, j) => {
    ...
    // formData.append('userId', ipt_user)
    ...
    // let url = ''
    // if (DATA.single == 'true') {
    //   url = 'https://open.pixsmart.cn:6802' + '/sdkMgr/sdk/sdkConcentrationDetectSingle'
    // } else {
    //   url = 'https://open.pixsmart.cn:6802' + '/sdkMgr/sdk/sdkConcentrationDetect'
    // }
    ...
    console.log('uploadFileRes', uploadFileRes.data, typeof uploadFileRes.data,)
    const resultData = uploadFileRes.data;
    console.log('resultData', typeof resultData,)
    console.log('resultData', resultData, typeof resultData,)
    // console.log(JSON.parse(resultData))
    let data = JSON.parse(resultData);
    console.log('data', typeof data,)
    console.log('data', data,)
    // if (uploadFileRes.statusCode === 200) {
    //   // 假设uploadFileRes.data里面包含你需要传递给handleChooseFile的信息
    //   const resultData = JSON.stringify(uploadFileRes.data);
    //   // console.log(resultData)
    //   let data = resultData;
    //   // 将上传结果传入handleChooseFile
    //   // 获取需要传递到下一个页面的数据
...
async function displayResults({ imgData, data, elapsedTime, filename }) {
  ...
  function genPageInfoEl() {
    console.log('++++++++++++++', data);
    console.log('data 类型:', typeof data); // 打印 data 的类型
  ...
  console.log('resultsContainerRef', resultsContainerRef.value)
  resultsContainerRef.value.appendChild(resultDiv)
...
const go_AI = async () => {
  ...
  //   const blob = chooseImageRes.tempFilePaths[0]
