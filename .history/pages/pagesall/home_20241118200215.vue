...
const fetchData = async () => {
  try {
    ...
    responses[0].items.forEach(item => {
      // 将活动信息添加到活动数组，去除 signup_member
      const { signup_member, ...activityInfo } = item; // 使用解构去除 signup_member
      listdata.activities.push({
        ...activityInfo, // 仅保留活动信息
        hours: 0,       // 初始化时字段
        minutes: 0,     // 初始化分字段
        seconds: 0      // 初始化秒字段
      });

      // 确保 signup_member 存在，并将总报名人数添加到数组
      if (signup_member) {
        listdata.signupMembers.push({
          total: signup_member.total || 0 // 避免 undefined
        });
      }
    });
