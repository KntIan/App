...
function btn_study() {
  if (this.item.status === 0) {
    this.$router.push('/write-homework') // 跳转到写作业页面，需按照项目实际路由配置修改路径
  } else if (this.item.status === 1 || this.item.status === 2) {
    this.$router.push('/homework-review-details') // 跳转到作业审阅详情页面，按实际配置调整路径
  }
}
</script>
