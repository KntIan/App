...
const compressImageFile = async (file) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (event) => {
      console.log('图像数据:', event.target.result);
      const img = new Image();
...
const go_AI = async () => {
  try {
    ...
    // 使用 uni.getFileSystemManager() 来读取文件内容
    ...
    const fileContent = await new Promise((resolve, reject) => {
      fs.readFile({
        ...
        success(res) {
          ...
          const blob = new Blob([byteNumbers], { type: fileInfo.type });
          resolve(blob);
        },
        fail(err) {
          reject(err);
    ...
    // 创建 File 对象
    const file = new File([fileContent], 'selected-image.jpg', {
      type: fileInfo.type, // 使用正确的 MIME 类型
    });
