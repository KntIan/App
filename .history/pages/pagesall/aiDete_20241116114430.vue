...
export default {
  ...
  methods: {
    async onCanvasReady() {
      const ctx = uni.createCanvasContext('aiBox', this);
      const store = useStore();
      const { ocrResult, wordMark } = store;
      const { label, box } = ocrResult;
      const { mark, centerMark, centerInfo, rateMark, rateInfo, areaMark, areaInfo, posMark, posInfo, conFMark, conFInfo } = wordMark;
