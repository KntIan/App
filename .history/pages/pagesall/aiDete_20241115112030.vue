...
async function handleChooseFile(file) {
  ...
  try {
    // 获取压缩后的 Blob
    // let blob = await compressAndResizeImageFile(file);
...
const choose = () => {
  uni.chooseImage({
    ...
    success: (res) => {
      ...
      // 创建新的 Image 对象
      const img = new Image();
      img.src = filePath; // 将选择的图片路径设置为 img 的源
      // 在canvas上绘制图像，进行尺寸转换
      img.onload = () => {
        // 在 canvas 上绘制图像，进行尺寸转换
        ctx.drawImage(img, 0, 0, 375, 500);

        // 将 canvas 内容压缩为 Blob 对象
        canvas.toBlob((blob) => {
          if (blob) {
            // 打印 Blob 对象的信息
            ...
            // 可以根据需要在此处使用 blob，例如上传或转换为 URL
            const url = URL.createObjectURL(blob);
            console.log('Blob URL:', url);
          } else {
            console.error('Blob creation failed.');
          }
        }, 'image/jpeg', 0.8); // 设置为 JPEG 格式，并指定质量为 0.8
      };
