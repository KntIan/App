...
const onFileChange = async (event) => {
  ...
  if (selectedFile) {
    await go_AI(); // Call go_AI when a file is selected
  }
};

// Define go_AI method to upload image
const go_AI = async () => {
  if (!selectedFile) {
  ...
  try {
    const response = await fetchAIRecognition(formData); // Call the API with form data
    if (response.code === 0) {
      console.log('上传成功:', response.data); // Handle successful upload
    } else {
      console.log('上传失败:', response.message); // Handle error message from API
    }
  } catch (error) {
    console.error('上传错误:', error); // Handle network errors
  }
};
</script>
