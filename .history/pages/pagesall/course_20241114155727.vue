  ...
  <timeline>
    <view class="line_list" v-for="(item, index) in ti_tea_room_list" :key="index">
      <timelineItem class="list_box_cour" :leftTime="item.title" :color="item.color" style="font-weight: 600; font-size: 28rpx; color: #000000">
        ...
        <view class="list_btn" :style="{ backgroundColor: item.status === 0 ? '#666666' : '#5791F2' }" @click="btn_study">
...
onMounted(async () => {
  try {
    ...
    // const nowCourse = courseListResponse.data.now.calligraphycourse || {};
    ...
    // if (courseListResponse.data.history?.calligraphycourse) {
    //   combinedCourses.push({
    //     ...courseListResponse.data.history.calligraphycourse,
    //     status: "作业已完成",
    //     color: "#666666",
    //   });
    // }
    ...
    // 例如：
    ti_tea_room_list.value = recentCourseResponse.items;
    console.log(ti_tea_room_list.value)
    // ti_tea_room_list.value = recentCourseResponse.items.map(
    //   ({
    //     title,
    //     teach,
    //     classroom,
