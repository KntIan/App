...
const fetchData = async () => {
  try {
    ...
    console.log(3, typeof listdataIndex.value); // 使用 .value 来获取实际的值
    if (Array.isArray(listdataIndex.value) && listdataIndex.value.length > 0) {
      store.setListdataIndex(listdataIndex.value); // 应使用 .value 进行传递
    } else {
      console.error('Invalid format for listdataIndex:', listdataIndex.value); // 同样确保在这里使用 .value
    }
    console.log(6)
