...
const go_AI = async () => {
  try {
    ...
    if (uploadFileRes.statusCode === 200) {
      const resultData = JSON.stringify(uploadFileRes.data); // 获取需要传递的数据
      uni.navigateTo({
        url: `/pages/pagesall/aiDete/aiResult?resultData=${encodeURIComponent(resultData)}`, // 跳转到 aiResult 页面并传递数据
      });
    }
  } catch (error) {
...
const handleLogin = () => {
  const jsonParams = {
    apiKey: user.value,
    apiSecret: pass.value,
  };

  fetch(`${server.value}/sdkMgr/auth/signin`, {
    method: 'POST',
    headers: {
