...
const fetchData = async () => {
  try {
    ...
    const listdata = responses[0].items.map(item => ({
      ...item,          // 保留原有的属性
      hours: 0,       // 增加时字段，初始化为0
      minutes: 0,     // 增加分字段，初始化为0
      seconds: 0      // 增加秒字段，初始化为0
    }));
    store.setListdata(listdata)
...
// 在 setup 中添加 watchEffect
watchEffect(() => {
  const currentDateTimestampInMilliseconds = new Date().getTime(); // 当前时间戳
  const now = Math.floor(currentDateTimestampInMilliseconds / 1000); // 转换为秒

  const newListData = listdata.value.map(item => {
    const newItem = { ...item }; // 创建新对象
    if (!isNaN(item.end_time)) {
      const timeDiff = item.end_time - now; // 计算时间差
      if (timeDiff > 0) {
        newItem.hours = Math.floor(timeDiff / 3600);
        newItem.minutes = Math.floor((timeDiff % 3600) / 60);
        newItem.seconds = timeDiff % 60;
      } else {
        newItem.hours = newItem.minutes = newItem.seconds = 0; // 时间到
      }
    } else {
      newItem.hours = newItem.minutes = newItem.seconds = 0; // 处理无效时间
    }
    return newItem;
  });

  // 更新 Pinia store
  store.setListdata(newListData);
});
// const updateCountdown = () => {
//   const currentDateTimestampInMilliseconds = new Date().getTime(); // 获取当前日期的时间戳（毫秒）
//   const now = Math.floor(currentDateTimestampInMilliseconds / 1000); // 转换为秒并取整
...
//       if (timeDiff > 0) {
//         item.hours = Math.floor(timeDiff / 3600); // 保持为数字
//         item.minutes = Math.floor((timeDiff % 3600) / 60); // 保持为数字
//         item.seconds = timeDiff % 60; // 保持为数字
//         console.log('item.hours', item.hours);
//         console.log('item.minutes', item.minutes);
//         console.log('item.seconds', item.seconds);
//       } else {
//         item.hours = item.minutes = item.seconds = 0; // 改为数字
//       }
//     } else {
//       item.hours = item.minutes = item.seconds = 0; // 如果无法解析则设为默认值
//     }
//   });
// }
