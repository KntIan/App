...
const onFileChange = async (event) => {
  selectedFile = event.target.files[0]; // 获取选中的文件
...
const uploadImage = async () => {
  ...
  formData.append("image", selectedFile); // 将选中的文件添加到 FormData

  try {
    const response = await fetchAIRecognition(formData); // 调用上传接口
    if (response.code === 0) {
      console.log("上传成功", response.data); // 上传成功的处理
    } else {
      console.error("上传失败", response.message); // 上传失败的处理
    }
  } catch (error) {
    console.error("上传出现错误", error); // 错误处理
  }
};

// 定义 go_AI 方法
const go_AI = () => {
  openFileChooser(); // 触发文件选择
};
</script>
