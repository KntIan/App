...
const search = async () => {
  ...
  let res = await checkUserFriend({ user_id: Number(searchValue.value) })
  console.log(res)
  if (res.code === 1) {
    TUICore.callService({
...
const btnscan = () => {
  uni.scanCode({
    success: async (res) => {
      ...
      let result = await checkUserFriend(params)
      if (result.code === 1) {
        TUICore.callService({
          serviceName: TUIConstants.TUISearch.SERVICE.NAME,
          method: TUIConstants.TUISearch.SERVICE.METHOD.SEARCH_USER,
          ...
          .then((result) => {
            searchResult.value.user.list = result.data
          })
          .catch((error) => {
            searchResult.value.user.list = []
            console.warn('search user error', error)
        ...
        TUICore.callService({
          ...
          method: TUIConstants.TUISearch.SERVICE.METHOD.SEARCH_GROUP,
          params: {
            groupID: Number(user_id),
          },
        })
          .then((result) => {
            searchResult.value.group.list = [result.data.group]
          })
          .catch((error) => {
            searchResult.value.group.list = []
            console.warn('search group error', error)
