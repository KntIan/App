...
async function handleChooseFile(file) {
  ...
  let blob = compressAndResizeImageFile(file, 375)
  console.log('choose blob', blob)
  let imageData = file2ImageData(blob)
  console.log('choose imageData', imageData)
  let data = evaluationImageData(imageData)
...
async function evaluationImageData(imgData) {
  return new Promise(async (r, j) => {
    ...
    // 上传图片
    const uploadFileRes = uni.uploadFile({
...
const go_AI = async () => {
  ...
  // 选择图片
  // const chooseImageRes =  uni.chooseImage({
  //   count: 1, // 最多选择1张图片
