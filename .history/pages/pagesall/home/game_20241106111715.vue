<template>
  ...
  <timeline>
    <timelineItem leftTime='开幕式' color="#84F257">
      <view class="tips">2024.08.01——2024.08.02</view>
    </timelineItem>
    <timelineItem leftTime='初赛' color="#84F257">
      <view class="tips">2024.08.01——2024.08.02</view>
    </timelineItem>
    <timelineItem leftTime='楷书组复赛' leftlist1="楷书组复赛" leftlist2="楷书组复赛" color="#F25757">
      <view class="tips">2024.08.01——2024.08.02</view>
    </timelineItem>
    <timelineItem leftTime='楷书组复赛' leftlist1="楷书组复赛" leftlist2="楷书组复赛" color="#F25757">
      ...
      <view class="infostr">
        ...
        <view class="paddbox">
          ...
          <view class="huatibox">
            ...
            <text style="font-weight: 400;font-size: 24rpx;color: #666666;font-style: normal; margin-left: 10rpx;">{{item.infotime}}</text>
          </view>
...
import { fetchMatchDetail } from '@/util/api'
...
export default {
  ...
  mounted() {
    const store = useStore()
    const index = store.selectedSwiperIndex; // 获取存储的下标
    console.log('获取的轮播图下标:', index);
  },
  methods: {
    async getMatchDetail(index) {
      try {
        console.log('请求比赛详情，当前下标:', index);
        const response = await fetchMatchDetail(index); // 调用接口并传入下标
        this.matchDetail = response; // 将获取到的数据保存到组件状态
