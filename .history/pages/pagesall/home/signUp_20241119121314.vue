...
export default {
  ...
  onLoad(options) {
    const paramsString = decodeURIComponent(options.params); // 解码参数
    const params = JSON.parse(paramsString); // 将 JSON 字符串解析为对象
    this.option = params
    console.log(params); // 查看解析后的参数
  ...
  methods: {
    async details() {
      try {
        let details = await fetchActivityCourseList(this.option);
      ...
      try {
        ...
        if (response.code === 1) {
          ...
          if (this.matchedDetails) {
            ...
            console.log('活动已报名:', this.matchedDetails);
          } else {
