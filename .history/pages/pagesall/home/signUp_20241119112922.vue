  ...
  methods: {
    async details() {
      try {
        ...
        if (matchedDetail) {
          ...
          this.matchedDetails = {
            ...matchedDetail, // 保留原有字段
            hours: 0, // 初始化时、分、秒字段，具体值可以根据你的逻辑设置
            minutes: 0,
            seconds: 0,
          };
    ...
    updateCountdown() {
      ...
      if (this.matchedDetails && !isNaN(this.matchedDetails.end_time)) {
        ...
        if (timeDiff > 0) {
          this.matchedDetails.hours = Math.floor(timeDiff / 3600);
          this.matchedDetails.minutes = Math.floor((timeDiff % 3600) / 60);
          this.matchedDetails.seconds = timeDiff % 60;
        } else {
          this.matchedDetails.hours = this.matchedDetails.minutes = this.matchedDetails.seconds = 0; // 时间到达
        }
      } else {
        console.warn('matchedDetails 无效，无法更新倒计时');
        this.matchedDetails.hours = this.matchedDetails.minutes = this.matchedDetails.seconds = 0; // 处理无效时间
      }
    },
