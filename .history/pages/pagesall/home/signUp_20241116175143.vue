  ...
  <view class="homePage">
    ...
    borderRadius: '50%'
            ...
            <view class="perbtn">
              ...
              <view class="gobtn" @click="btnsignup" :style="item.hadSignup ? { background: '#ff9e02' } : { backgroundColor: '#999999' }">
                {{ item.hadSignup ? '立即报名' : '已报名' }}
              </view>
            </view>
          </view>
        </view>
...
import { signUpActivity } from '@/utils/api'
export default {
  data() {
    return {
      //   images: [
  ...
  methods: {
    ...
    async btnsignup() {
      ...
      try {
        const response = await signUpActivity({ user_id: user_id, activity_id: this.activityId });
        ...
        if (response.code = 1) {
          ...
          const currentActivity = this.listdata.find(item => item.id === activityId);
          if (currentActivity) {
            currentActivity.isSignedUp = true;
          }
        }
