...
export default {
  ...
  methods: {
    ...
    async btnsignup() {
      // 从本地存储获取用户 ID
      const userId = uni.getStorageSync('userId'); // 假设用户 ID 存储在 'userId' 下
      const activityId = this.learnList.info.id; // 获取当前活动的 ID
      ...
      try {
        const response = await signUpActivity({ userId, activityId });
