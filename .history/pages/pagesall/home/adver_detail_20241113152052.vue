<template>
  <view class="code_page">
    ...
    <view class="detail_title">
      <text style="font-weight: 500;font-size: 32rpx;color: #000000;">快来参加书法协会的比赛吧</text>
...
export default {
  ...
  onLoad(options) {
    ...
    this.messageList = store.messageList; // 从 Store 中获取 messList 并赋值
    console.log(this.messageList)
    const messageId = options.id; // 获取传入的公告 ID
    this.selectedMessage = this.messageList.find(message => message.id === messageId); // 查找对应 ID 的公告

    console.log(this.selectedMessage); // 打印选中的公告
  },
  methods: {
    goback() {
      uni.navigateBack()
