<template>
  <view class="container">
    ...
    <view class="section">
      <view>
        <activity-item v-for="(item, index) in listdata" :key="index" :online="item.is_online" :techerimg="item.avatar" :biaoti="item.title" :laoshi="item.laoshi" :star="item.s_time" :end="item.e_time" :hearimg="item.hearimg" :teachername="item.teachername" :hours="item.hours" :minutes="item.minutes" :seconds="item.seconds" :pers="item.signup_member.total" :money="item.price" :imgs="item.imgs" :signedUp="item.signedUp" @signup="handleSignup" />
      </view>
