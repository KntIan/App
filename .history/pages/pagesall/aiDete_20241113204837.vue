...
async function evaluationImageData(imgData) {
  return new Promise(async (r, j) => {
    ...
    // console.log(uploadFileRes.thumb)
    // uni.setStorageSync('', uploadFileRes.data.thumb);


    // const imgData = uploadFileRes.thumb
...
const go_AI = async () => {
  ...
  uni.chooseImage({
    ...
    success: (res) => {
      ...
      const maxWidth = 1170 // 最大宽度
      const maxHeight = 1170 // 最大高度
      let width = 0 // 图片宽度
      let height = 0 // 图片高度
      let ratio = 1 // 比例
      // 获取图片信息
      uni.getImageInfo({
        ...
        success: (result) => {
          ...
          uni.compressImage({
            ...
            success: (compressRes) => {
              console.log('图片地址:' + compressRes.tempFilePath)
            },
            fail: (err) => {
              console.log(err)
            }
