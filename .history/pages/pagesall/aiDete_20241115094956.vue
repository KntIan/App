...
// 				// DATA.token = token
// 				DATA.token =
// 					'eyJhbGciOiJIUzI1NiJ9.eyJwYXlsb2FkIjp7InN5c1ByZVJvbGUiOmZhbHNlLCJvYyI6bnVsbCwiZSI6MTczMTQyMDA3ODYyMSwidCI6MSwidSI6Inl1bnBlbmdrZWppIiwiYWNzIjpudWxsLCJsdyI6bnVsbCwic3ByIjpmYWxzZX19.Hh969-k8whchGCoqlBAg5VGk135gG03z9Lnpgq-S73Y'
// 				console.log('登录成功')
...
async function handleChooseFile(file) {
  ...
  try {
    // 获取压缩后的 Blob
    let blob = await compressAndResizeImageFile(file, 375);
    ...
    console.error('处理文件时出错:', error); // 捕获并打印任何错误
  }
}

//压缩并修改上传图片的尺寸
// function compressAndResizeImageFile(file, targetWidth, targetHeight) {
...
//           targetHeight = targetWidth / aspectRatio; // 根据宽度计算高度
//         } else if (!targetWidth && targetHeight) {
//           targetWidth = targetHeight * aspectRatio; // 根据高度计算宽度
//         } else if (!targetWidth && !targetHeight) {
//           targetWidth = img.width; // 没有指定任何尺寸，使用原始尺寸
...
function compressAndResizeImageFile(targetWidth, targetHeight) {
  return new Promise((resolve, reject) => {
    // 使用选择器选择图片
    uni.chooseImage({
      count: 1,
      success: (chooseResult) => {
        const filePath = chooseResult.tempFilePaths[0]; // 获取选择的图片路径
        ...
        uni.getImageInfo({
          ...
          success: (imgInfo) => {
            ...
            // 计算调整后的宽高比
            if (targetWidth && !targetHeight) {
              targetHeight = targetWidth * (height / width); // 根据宽度计算高度
            } else if (!targetWidth && targetHeight) {
              targetWidth = targetHeight * (width / height); // 根据高度计算宽度
            } else if (!targetWidth && !targetHeight) {
              targetWidth = width; // 没有指定任何尺寸，使用原始尺寸
              targetHeight = height;
            }
            ...
            uni.compressImage({
              ...
              success: (compressResult) => {
                console.log(compressResult.tempFilePath)
                resolve(compressResult.tempFilePath); // 返回压缩后的图片路径
...
// 将一个 Blob 对象转换为一个包含图像及其相关信息的 Promise 对象
function file2ImageData(blob) {
  return new Promise((r, j) => {
    const compressFile = new File([blob], 'tmp.jpg', {
      type: blob.type
    })
    const reader = new FileReader()
    // console.log(reader)
...
async function evaluationImageData(imgData) {
  return new Promise(async (r, j) => {
    ...
    const uploadFileRes = await uni.uploadFile({
      url: 'http://admin.zexishuhua.com/api/common/detecthomework', // 替换为你的上传接口地址
    ...
    let resultData = uploadFileRes.data;
...
async function displayResults({
  ...
  console.log(rightDiv, typeof resultDiv)
  uni.navigateTo({
  ...
  function draw() {
    ...
    ocrResults.forEach((ocr, i) => {
      ...
      let x = ocr.points[0] * 0.29
      let y = ocr.points[1] * 0.29
      let width = (ocr.points[4] - ocr.points[0]) * 0.29
      let height = (ocr.points[5] - ocr.points[1]) * 0.29
      // console.log('ocr', ocr, x, y, width, height)
      let wordInfo = word.mark.toFixed(0)
      let score = parseFloat(wordInfo);
      ...
      if (
        ...
        ctx.fillText(displayText, x + 12, y + 25)
        ...
        ctx.strokeRect(x, y, width, height)
        // ctx.fillText(`${word.mark.toFixed(0)}`, x + 12, y + 25)
        ctx.fillText(displayText, x + 12, y + 25)
      }
    })
...
const choose = () => {
  uni.chooseImage({
    ...
    success: (res) => {
      ...
      blob.value = res.tempFilePaths[0]
