...
function compressImageFile(file) {
  console.log('++++++5+5+', file)
  return new Promise((resolve, reject) => {
    // 使用 uni.compressImage API 压缩图片
    uni.compressImage({
      src: file, // 源图片的路径
      ...
      fail: (error) => {
        reject(error); // 处理压缩失败的情况
      }
    });
  });
}
// function compressImageFile(file) {
//   // console.log('compressImageFile', file)
//   return new Promise((resolve, reject) => {
//     const reader = new FileReader()
//     // console.log('----------------------', reader)
//     reader.onload = function (event) {
  ...
  uni.chooseImage({
    ...
    success: (res) => {
      ...
      if (res.tempFiles.length > 0) {
        let filePath = res.tempFiles[0].tempFilePath; // 获取正确的图片路径
        // 压缩图片
        uni.compressImage({
          src: filePath, // 源图片的路径
          quality: 80, // 压缩质量，范围 0 - 100
          success: (compressRes) => {
            ...
            blob.value = compressRes.tempFilePath; // 将压缩后的路径赋值给 blob
            handleChooseFile(compressRes.tempFilePath); // 处理压缩后的图片文件
          },
          fail: (error) => {
            console.error('压缩失败:', error); // 如果压缩失败，输出错误信息
          }
        });
      } else {
    ...
    fail: (error) => {
      console.error('选择图片失败:', error); // 如果选择图片失败，输出错误信息
    }
  });
