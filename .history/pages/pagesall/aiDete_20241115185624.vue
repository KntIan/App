...
export default {
  ...
  onLoad() {
    ...
    // 假设 store.canvasNode 是您的 DOM 结构的根元素
    const canvasElement = store.canvasNode; // 直接访问存储的 DOM 节点
    console.log('canvasNode:', canvasElement);
    if (canvasElement) {
      // 获取 canvas-id
      const canvasId = canvasElement.getAttribute('canvas-id');
      console.log('获取的 canvas-id:', canvasId);
    } else {
      console.log('canvasNode 尚未定义');
    }
