  ...
  methods: {
    ...
    drawMask(canvasId, label, base64String) {
      ...
      ctx.draw(false, () => {
        ...
        const canvasElement = query.select('#aiBox'); // 获取 canvas 元素
        console.log(canvasElement, typeof canvasElement)
        // const divElement = document.querySelector('#aiBox');
        // console.log(divElement, typeof divElement)
        store.setResultDiv(canvasElement);
        if (canvasElement) {
          store.setResultDiv(canvasElement);
        } else {
        ...
        // store.setCanvasNode(canvasElement, typeof canvasElement); // 保存元素到 store
        uni.navigateTo({
