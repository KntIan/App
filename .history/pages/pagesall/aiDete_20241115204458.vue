...
export default {
  ...
  mounted() {
    ...
    const canvasStore = store.useCanvasStore(); // 使用 canvas store

    this.getToken(); // 在组件挂载时获取 token

    const canvasElement = this.$refs.myCanvas; // 获取 canvas 元素
    canvasStore.setCanvasElement(canvasElement); // 保存元素到 store
  },
  methods: {
