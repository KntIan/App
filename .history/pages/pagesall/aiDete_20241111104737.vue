...
const getToken = () => {
  // 这里应该是从您的应用程序存储中获取 token，例如从 localStorage 或 Vuex
  return 'YOUR_ACCESS_TOKEN'; // 替换为真实的 token
};
...
const go_AI = async () => {
  try {
    ...
    const uploadFileRes = await uni.uploadFile({
      ...
      name: 'file', // 上传接口的字段名
      header: {
        'Authorization': `Bearer ${getToken()}`, // 使用 Bearer Token 进行认证
      },
    });
    ...
    // 如果需要，可以在这里进行 AI 识别的后续调用
    // const recognitionResponse = await fetchAIRecognition(uploadFileRes.data);
    // console.log('AI识别结果', recognitionResponse);
  } catch (error) {
