        ...
        <view class="content">
          <view v-if="current === 0">
            <view
              ...
              v-if="onlineInProgress.length > 0"
            >
              <activity-item
                v-for="(item, index) in onlineInProgress"
                :key="index"
            ...
            >
              <activity-item
                v-for="(item, index) in onlineReviewofPastEvents"
                :key="index"
                :online="item.is_online"
                :techerimg="item.avatar"
                :biaoti="item.title"
          ...
          <view v-if="current === 1">
            <view
              style="display: grid; justify-items: center"
              v-if="offlineOngoing.length > 0"
            >
              <activity-item
                v-for="(item, index) in offlineOngoing"
                :key="index"
                :online="item.is_online"
                :techerimg="item.avatar"
                :biaoti="item.title"
            ...
            <view>
              ...
              >
                <activity-item
                  v-for="(item, index) in offlineReviewofPreviousEvents"
                  :key="index"
                  :online="item.is_online"
                  :techerimg="item.avatar"
                  :biaoti="item.title"
  ...
  methods: {
    async fetchActivityData() {
      try {
        ...
        console.log(response, afoot)
        // if (
        //   response.items &&
        //   response.items.length === 0 &&
        //   afoot.items &&
        //   afoot.items.length === 0
        // ) {
        //   // 返回特定值以表明没有更多数据
