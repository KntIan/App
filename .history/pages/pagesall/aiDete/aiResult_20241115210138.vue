<template>
  ...
  <view class="results-container" v-if="resultsContainer">
    <slot :content="resultsContainer"></slot>
  </view>
...
const canvasRef = ref(null); // 定义 canvas 的引用
...
onMounted(() => {
  console.log(store.canvasNode)
  resultsContainer.value.appendChild(store.canvasNode); // 假设 resultDivData 是存储的节点
  // resultsContainer.value = store.canvasNode; // 假设 resultDivData 是存储的节点

  // 如果找不到节点，可以遍历一下以确认是否成功获取
  // if (!resultsContainer.value) {
  // console.error("未能从 store 获取节点");
