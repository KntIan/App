...
// 在组件挂载后执行
onMounted(() => {
  console.log('onMounted', store.resultDivData, typeof store.resultDivData);
  ...
  if (savedResultDiv) {
    ...
    console.log(parser)
    const doc = parser.parseFromString(savedResultDiv, 'text/html');
    const preElement = doc.querySelector('.vertical-container');
    console.log(1111111, preElement)
    if (preElement) {
      const textContent = preElement.textContent;

      // 使用正则表达式提取分数
      const scores = {
        卷面分: parseInt(textContent.match(/卷面得分 (\d+)/)[1]),
        重心得分: parseInt(textContent.match(/重心得分 (\d+)/)[1]),
        形态得分: parseInt(textContent.match(/形态得分 (\d+)/)[1]),
        大小得分: parseInt(textContent.match(/大小得分 (\d+)/)[1]),
        位置得分: parseInt(textContent.match(/位置得分 (\d+)/)[1]),
        笔画得分: parseInt(textContent.match(/笔画得分 (\d+)/)[1]),
      };

      // 对卷面分进行分类
      const 卷面分等级 = scores.卷面分 >= 60 ? '优' : '良';
      ...
      const 重心得分等级 = getScoreLevel(scores.重心得分);
      const 形态得分等级 = getScoreLevel(scores.形态得分);
      const 大小得分等级 = getScoreLevel(scores.大小得分);
      const 位置得分等级 = getScoreLevel(scores.位置得分);
      const 笔画得分等级 = getScoreLevel(scores.笔画得分);

      console.log('卷面分等级:', 卷面分等级);
      console.log('重心得分等级:', 重心得分等级);
      console.log('形态得分等级:', 形态得分等级);
      console.log('大小得分等级:', 大小得分等级);
      console.log('位置得分等级:', 位置得分等级);
      console.log('笔画得分等级:', 笔画得分等级);
    }
  }
});
