  ...
  methods: {
    ...
    draw(canvas, ctx, base64Mask, wordDetail) {
      ...
      canvas.addEventListener("click", function (event) {
        ...
        self.render(canvas, ctx, base64Mask, wordDetail);
      });
      this.render(canvas, ctx, base64Mask, wordDetail);
    },
    render(canvas, ctx, base64Mask, wordDetail) {
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      ctx.drawImage(this.img, 0, 0, this.img.width, this.img.height, 0, 0, canvas.width, canvas.height);

      // 其他的绘制逻辑...
      // 使用 this.ocrResults 和 this.wordMarks 来处理 OCR 结果和单词标记
    },
