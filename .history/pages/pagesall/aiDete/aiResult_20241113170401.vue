...
export default {
  setup() {
    const store = useStore(); // 使用 useStore 进行状态管理访问
    ...
    return {
      stu_List,
      score_List,
      store, // 将 store 作为返回值，可以在模板中使用
    };
  },

  mounted() {
    const store = useStore();
    // 这里可以访问 resultsContainer
    console.log('resultsContainer:', this.$refs.resultsContainer);

    // 例如，添加一个新的元素
    const newElement = document.createElement('div');
    // 从 store 中获取 resultDivData
    newElement.appendChild(store.resultDivData);

    // 确保 resultsContainer 存在
    if (this.$refs.resultsContainer) {
      this.$refs.resultsContainer.appendChild(newElement);
    }
  },
