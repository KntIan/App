...
onMounted(() => {
  ...
  //插入节点
  const savedResultDiv1 = store.resultDivData;

  // 删除 class 为 vertical-container 的节点
  if (savedResultDiv1) {
    const verticalContainer = savedResultDiv1.querySelector('.vertical-container');
  ...
  if (resultsContainer.value && savedResultDiv1) {
    resultsContainer.value.appendChild(savedResultDiv1);
  }
  ...
  if (savedResultDiv) {
    ...
    if (preElement) {
      const scoreText = preElement.textContent;
