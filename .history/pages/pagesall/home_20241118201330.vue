...
const fetchData = async () => {
  try {
    const responses = await Promise.all([
      fetchActivityCourseList(token, params), // 传入 page, limit, isindex
    ...
    const listdata = responses[0].items.map(item => ({
      ...
      totalSignUps: item.signup_member && item.signup_member.total ? item.signup_member.total : 0 // 确保总报名人数存在
    }));
