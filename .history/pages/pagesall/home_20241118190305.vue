...
const updateCountdown = () => {
  ...
  const updatedListdata = listdata.value.map(item => {
    const newItem = { ...item };
    if (!isNaN(item.end_time)) {
      ...
      if (timeDiff > 0) {
        ...
        newItem.hours = newItem.minutes = newItem.seconds = 0;
      }
    } else {
      newItem.hours = newItem.minutes = newItem.seconds = 0;
    }
    console.log('Updated time:', newItem.hours, newItem.minutes, newItem.seconds);
    return newItem;
  });

  store.setListdata(updatedListdata); // 更新 store
};
const btnscan = () => {
  uni.scanCode({
    success: (res) => {
