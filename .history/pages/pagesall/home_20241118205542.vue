...
//     }));
//     console.log(3, listdataIndex)
//     if (Array.isArray(listdataIndex) && listdataIndex.length > 0) {
//       store.setListdataIndex(listdataIndex);
//     } else {
//       console.error('Invalid format for listdataIndex:', listdataIndex);
//     }
//     console.log(6)
...
const fetchData = async () => {
  try {
    // 单独请求活动课程
    const activityCourses = await fetchActivityCourseList(params).catch(error => {
      console.error('获取活动课程失败:', error);
      return null; // 或者根据业务需求返回一个默认值
    });

    // 单独请求横幅列表
    const banners = await fetchBannerList().catch(error => {
      console.error('获取横幅列表失败:', error);
      return []; // 返回一个空数组作为默认值
    });

    // 单独请求新闻列表
    const news = await fetchNewsList().catch(error => {
      console.error('获取新闻列表失败:', error);
      return []; // 返回一个空数组作为默认值
    });

    // 单独请求个人风格
    const userStyle = await fetchMyStyle().catch(error => {
      console.error('获取个人风格失败:', error);
      return {}; // 返回一个空对象作为默认值
    });

    // 处理返回的数据
    if (activityCourses) {
      store.setListdataIndex(activityCourses.items);
    }
    store.setImages(banners);
    store.setInfoList(news);
    studentAndstyle.value = userStyle.data ? userStyle.data : {};

  } catch (error) {
    console.error('请求过程中发生未知错误:', error);
  }
};
...
const updateCountdown = () => {
  ...
  store.setListdataIndex(listdataIndex); // 更新 store
};
const btnscan = () => {
  uni.scanCode({
    success: (res) => {
