...
const blobToFile = async (blobUrl) => {
  ...
  const file = new File([blob], 'avatar.png', { type: blob.type }); // 创建文件对象
  console.log(file); // 打印文件对象
  return file;
...
const takePhoto = () => {
  uni.chooseImage({
    ...
    success: async (res) => {
      ...
      this.avatarUrl = filePath; // 更新头像URL为选中的图片
      console.log(filePath);
      const file = await this.blobToFile(filePath); // 将Blob URL转换为文件对象
      console.log(file);
      await this.uploadImage({ file: file }); // 上传图片到后台
    },
    fail: (error) => {
