    ...
    <view class="form">
      ...
      <view class="input_po">
        <view class="uni-list-cell-db input">
          <picker mode="selector" @change="onClassChange" :value="multiIndex" :range="multiArray.map(item => item.title)" :disabled="isDisabled2">
...
onMounted(async () => {
  ...
  try {
    ...
    // 处理出生年月日
    const birthDate = new Date(userInfo.data.birthDate);
    ...
    // 处理校区
    const campus = store.campusData.find(item => item.id === userInfo.data.campusId);
    if (campus) {
    ...
    // 处理班级
    const classItem = store.classData.find(item => item.id === userInfo.data.classId);
    if (classItem) {
      const classIndex = store.classData.indexOf(classItem);
      multiIndex.value = [0, classIndex];
    }
  } catch (error) {
