...
// 创建文件对象
const createFileObjectFromPath = (filePath) => {
    ...
    uni.getFileInfo({
      ...
      success: (info) => {
        ...
        uni.getFileSystemManager().readFile({
          ...
          success: (fileRes) => {
            // 创建一个File对象
            const file = new File([fileRes.data], info.name, {
              type: info.type,
            });
            resolve(file); // 返回生成的文件对象
          },
          fail: (error) => {
            reject('读取文件失败: ' + error);
          }
...
const takePhoto = () => {
  uni.chooseImage({
    ...
    success: async (res) => {
      ...
      avatarUrl.value = filePath; // 更新头像URL为选中的图片
      console.log(filePath);
      const file = await blobToFile(filePath); // 将Blob URL转换为文件对象
      console.log(file);
      await uploadImage({ file: file }); // 上传图片到后台
    },
    fail: (error) => {
...
const pickImageFromAlbum = () => {
  uni.chooseImage({
    ...
    success: async (res) => {
      ...
      try {
        const file = await createFileObjectFromPath(filePath);
        console.log(file);
        await uploadImage({ file }); // 上传文件
      } catch (error) {
        console.error('文件对象构建失败:', error);
      }
    },
    fail: (error) => {
