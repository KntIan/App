        ...
        <view class="form">
          ...
          <view class="textlogin">
            <view class="img1" @click="chooseAvatar">
              <image :src="avatarUrl1" style="width: 192rpx;height: 192rpx;border-radius: 50%;" mode="aspectFill" />
...
const getlogin = async () => {
  try {
    ...
    if (avatarUrl.value) { // 确保头像 URL 存在
      const response = await updateAvatar({ avatar: avatarUrl.value });
      // console.log('头像上传成功:', response);

      if (response.code === 1) {
        console.log(store.userinfo.avatar, '+++++++++868686')
