<template>
  <view class="class-group">
    ...
    <view style="padding: 0 28rpx;">
      <TUIChat1 v-if="classGroupListListLength.length>0" />
...
export default {
  ...
  onLoad() {
    ...
    classGroup
    const classGroupList = uni.getStorageSync('classGroup');
    console.log('+5+5+5+5+6666', JSON.parse(classGroupList))
    this.classGroupListListLength = JSON.parse(classGroupList) // 如果存储的是一个 ref 对象, 使用 .value
    console.log('+5+5+5+5+33', this.classGroupListListLength)
  },
  methods: {
    mycode() {
      uni.navigateTo({
    ...
    handleLengthUpdate(length) {
      conversationListLength.value = length; // 更新长度信息
    },
    goback() {
      uni.navigateBack()
    }
