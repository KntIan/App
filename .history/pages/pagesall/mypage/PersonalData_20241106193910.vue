...
const takePhoto = () => {
  uni.chooseImage({
    ...
    success: async (res) => {
      ...
      console.log(file);
      await uploadImage({ file: filePath }); // 上传图片到后台
    },
    fail: (error) => {
...
const pickImageFromAlbum = () => {
  uni.chooseImage({
    ...
    success: async (res) => {
      ...
      // 假设 blob 是从 filePath 读取的 Blob 对象
      const response = await fetch(filePath);
      const blob = await response.blob(); // 获取 Blob 对象

      const fileName = `avatar_${Date.now()}.png`; // 动态生成文件名
      const file = new File([blob], fileName, { type: blob.type }); // 创建 File 对象
      console.log(file);

      avatarUrl.value = file.name; // 更新头像URL为选中的图片
      await uploadImage({ file: file.name }); // 上传文件而不是路径
    },
    fail: (error) => {
