...
export default {
  ...
  onLoad() {
    ...
    const storedTeacherListLength = uni.getStorageSync('teacherListLength');
    this.teacherListLength = uni.getStorageSync('teacherListLength')

    // 确保使用 .value 获取真正的长度
    if (storedTeacherListLength && storedTeacherListLength.value !== undefined) {
      this.teacherListLength = storedTeacherListLength.value; // 如果存储的是一个 ref 对象, 使用 .value
      console.log('+5+5+5+5+33', this.teacherListLength)
    } else {
      this.teacherListLength = storedTeacherListLength; // 或者直接将其赋值
      console.log('+5+5+5+5+22', this.teacherListLength)
    }

    console.log('+5+5+5+5+888', storedTeacherListLength._value)
