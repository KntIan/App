...
export default {
  ...
  onReachBottom() {
    ...
    this.getAIassessmentList().then(() => {
      ...
      // 检查新的数组长度
      if (newLength1 === currentLength && newLength2 === currentLength2) {
        // 如果数组长度没有增加，重置加载状态
        this.isLoading = true;
      } else {
  ...
  methods: {
    ...
    async loadMyTemporaryHomework() {
      try {
        const response = await fetchMyTemporaryHomework(params); // 调用 API 请求
        const HistoryHomework = await fetchMyHistoryHomework(params1); // 调用 API 请求
