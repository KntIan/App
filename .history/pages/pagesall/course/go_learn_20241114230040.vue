<template>
  ...
  <video @play="playAudio" @pause="pauseAudio" @ended="stopAudio" class="learn_video" :src="learnList.font_data[0].video" muted='true'></video>
        ...
        <view class="act_Img">
          ...
          <view class="act_time">
            ...
            <view class="time_textbox">
              <text style="font-size: 20rpx;padding: 3rpx 5rpx 3rpx 5rpx;border-radius: 5rpx;">{{ item.hours}}</text>
              <text style="font-size: 20rpx;background-color	:#fff;color: #F53C38;">:</text>
              <text style="font-size: 20rpx;padding: 3rpx 5rpx 3rpx 5rpx;border-radius: 5rpx;">{{ item.minutes}}</text>
              <text style="font-size: 20rpx;background-color:#fff;color: #F53C38;">:</text>
              <text style="font-size: 20rpx;padding: 3rpx 5rpx 3rpx 5rpx;border-radius: 5rpx;">{{ item.seconds}}</text>
            </view>
          </view>
        </view>
      </view>
  ...
  methods: {
    ...
    async loadHomeworkAudio() {
      try {
        ...
        this.learnList = response.items || []; // 更新数据
        console.log('教师信息:+++++++', this.learnList); // 打印获取的数据

        console.log(this.learnList.length > 0)

        // 检查 learnList 是否存在，并赋值 audioSrc
        if (this.learnList[0].font_data) {
          const audioSrc = this.learnList[0].font_data.voice_url; // 获取音频 URL
          this.audioContext.src = audioSrc; // 设置音频源地址
        }


        console.log(this.learnList[0])
        // 赋值 finish_time 给 homeworkFinishTime
        this.homeworkFinishTime = this.learnList[0].finish_time; // 假设有 finish_time 字段



        console.log('作业截止时间: ', this.homeworkFinishTime); // 打印作业截止时间
      } catch (error) {
    ...
    updateCountdown() {
      this.listdata.forEach((e, index) => {
        ...
        // 假设 e.end 这是您结束时间的字符串，需转换为 Date 对象
        let end = new Date(e.end); // 格式需为可解析的字符串，例如 "2024-11-13 12:00:00"
