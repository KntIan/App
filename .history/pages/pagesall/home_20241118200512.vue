...
const fetchData = async () => {
  try {
    ...
    // 创建一个对象来存储活动信息和报名人数信息
    const listdata = {
      activities: [], // 活动信息数组
      signupMembers: [] // 报名人数信息数组
    };
    responses[0].items.forEach(item => {
      // 使用解构赋值从活动 item 中去除 signup_member 
      const { signup_member, ...activityInfo } = item; // 解构赋值，去掉 signup_member

      // 将活动信息添加到活动数组
      listdata.activities.push({
        ...activityInfo,
        hours: 0,
        minutes: 0,
        seconds: 0,
      ...
      // 确保 signup_member 存在，并添加到报名人数数组
      if (signup_member) {
        listdata.signupMembers.push({
          total: signup_member.total || 0, // 报名人数
          items: signup_member.items || []   // 报名成员信息
        });
      }
    });
...
const updateCountdown = () => {
  ...
  store.setListdata(listdata); // 更新 store
};
const btnscan = () => {
  uni.scanCode({
    success: (res) => {
