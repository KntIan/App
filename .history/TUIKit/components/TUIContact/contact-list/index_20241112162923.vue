		...
		<li v-for="(contactListObj, key) in contactListMap" :key="key" class="tui-contact-list-item">
			...
			<ul :class="['tui-contact-list-item-main', currentContactListKey === key ? '' : 'hidden']">
				<li v-for="contactListItem in contactListObj.list" :key="contactListItem.renderKey"
					class="tui-contact-list-item-main-item" :class="['selected']" @click="selectItem(contactListItem)">
	...
	<ul v-else class="tui-contact-list">
		<li v-for="(item, key) in contactSearchResult" :key="key" class="tui-contact-list-item">
			<div v-if="item.list[0]" class="tui-contact-search-list">
				...
				<div v-for="(listItem, index) in item.list" :key="index" class="tui-contact-search-list-item"
					:class="['selected']" @click="selectItem(listItem)">
