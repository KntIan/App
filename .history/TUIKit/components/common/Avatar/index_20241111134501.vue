<template>
  <div class="avatar-container" style="width: 44px; height: 44px;" :style="{
      width: avatarSize,
      height: avatarSize,
      borderRadius: avatarBorderRadius,
    }">
    <template v-if="isUniFrameWork">
      <image v-if="!loadErrorInUniapp" class="avatar-image" :src="avatarImageUrl || defaultAvatarUrl" @load="avatarLoadSuccess" @error="avatarLoadFailed" />
      <image v-else class="avatar-image" :src="defaultAvatarUrl" @load="avatarLoadSuccess" @error="avatarLoadFailed" />
    </template>
    <img v-else class="avatar-image" :src="avatarImageUrl || defaultAvatarUrl" @load="avatarLoadSuccess" @error="avatarLoadFailed">
