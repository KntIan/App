<template>
  ...
  <div>
    <div v-if="props.marker === 'GROUP'" ref="conversationListInnerDomRef" class="tui-conversation-list">
      ...
      <div v-for="(conversation, index) in conversationList1" :id="`convlistitem-${index}`" :key="index" :class="[
    ...
    <div v-if="props.marker === 'C2C'" ref="conversationListInnerDomRef" class="tui-conversation-list">
      <ActionsMenu v-if="isShowOverlay" :selectedConversation="currentConversation" :actionsMenuPosition="actionsMenuPosition" :selectedConversationDomRect="currentConversationDomRect" @closeConversationActionMenu="closeConversationActionMenu" />
...
const conversationList1 = ref<IConversationModel[]>([]);
...
function onConversationListUpdated(list: IConversationModel[]) {
  ...
  // Initialize an empty array for GROUP type conversations

  // Iterate through the list to separate conversations
  list.forEach((conversation) => {
    if (conversation.type === "GROUP") {
      // If the type is GROUP, push to conversationList1
      conversationList1.value.push(conversation);
    } else {
      // Otherwise, we push it to the original conversationList
      conversationList.value.push(conversation);
    }
  });

  // Log the group conversations
  console.log("Group Conversations: ", conversationList1);

  // Store conversationList1 to local storage
  uni.setStorage({
    key: "conversationList1", // local storage key
    data: conversationList1, // data to be stored
    success: function () {
      console.log("conversationList1 has been stored successfully.");
    },
    fail: function () {
      console.error("Failed to store conversationList1.");
    },
  });
}
