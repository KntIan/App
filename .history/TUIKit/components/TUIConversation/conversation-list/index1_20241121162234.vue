<template>
  <div>
    <div v-if="classGroupListListLength.length > 0" ref="conversationListInnerDomRef" class="tui-conversation-list">
...
let classGroupListListLength = ref();
onMounted(() => {
  ...
  const classGroupList = uni.getStorageSync("classGroupList");
  console.log("+5+5+5+5+6666", JSON.parse(classGroupList));
  classGroupListListLength.value = JSON.parse(classGroupList); // 如果存储的是一个 ref 对象, 使用 .value
  console.log("+5+5+5+5+33", classGroupListListLength.value);
});
...
function onConversationListUpdated(list: IConversationModel[]) {
  ...
  uni.setStorageSync("classGroup", JSON.stringify(conversationList.value));
